...
- convert Stitch HTML/CSS and Figma node-tree exports into UGUI hierarchy when provided
...
- Stitch HTML/CSS와 Figma node-tree export가 제공되면 UGUI hierarchy로 변환합니다
...
```text
Convert this Stitch HTML/CSS export into a UGUI layout.
Use the export as hierarchy input, not as a browser runtime target.
Map sections, flex containers, repeated cards, scroll regions, and overlays into parent-owned UGUI containers and reusable prefabs.
Do not use direct Stitch API access for this task.
```

```text
Convert this Figma node-tree export into UGUI.
Treat frames, groups, components, instances, and auto-layout nodes as structure signals.
Keep DESIGN.md or token files as styling sources only, then verify the result with screenshots.
Do not use direct Figma API access for this task.
```
