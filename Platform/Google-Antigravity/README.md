...
- keep raster item candidates in a candidate item ledger until review
- record item-level UI rects only after parent ownership and split/keep reason are clear
...
- For UGUI conversion, use Stitch HTML/CSS exports and Figma node-tree exports as structured hierarchy input, separate from DESIGN.md/design-token style inputs.
- Direct Stitch/Figma API integration is out of scope unless explicitly requested.

## Mockup Stack Routing / 목업 스택 라우팅

Choose the UI stack before realization, create and approve the neutral `mockup-layout-plan/v2` layer-to-layout tree, then use [`ui-stack-selection.md`](../../unity-mcp-ui-layout/references/ui-stack-selection.md), [`ui-toolkit-build-workflow.md`](../../unity-mcp-ui-layout/references/ui-toolkit-build-workflow.md), and [`ui-toolkit-from-mockup-example.md`](../../examples/ui-toolkit-from-mockup-example.md). The canonical machine-readable examples are [`mockup-layout-plan-prefab-example.yaml`](../../examples/mockup-layout-plan-prefab-example.yaml) and [`mockup-layout-plan-ui-toolkit-example.yaml`](../../examples/mockup-layout-plan-ui-toolkit-example.yaml), based on [`mockup-layout-plan.yaml`](../../templates/mockup-layout-plan.yaml).

realization 전에 UI stack을 고르고 중립 `mockup-layout-plan/v2` layer-to-layout tree를 승인한 뒤 위 경로를 사용합니다. UGUI realization은 Transform/RectTransform과 prefab 의도로, UI Toolkit realization은 visual tree, UXML, USS, VisualTreeAsset template 의도로 이어집니다. runtime host가 필요할 때만 host GameObject/UIDocument를 추가하고, stack 선택부터 screenshot/console completion evidence까지 기록하기 전에는 완료하지 않습니다.
...
- UGUI 구조 변환 시, Stitch HTML/CSS export 및 Figma node-tree export는 DESIGN.md/디자인 토큰 기반 스타일 소스와 분리된 계층 입력으로 사용합니다.
- Stitch/Figma API 직접 연동은 별도 요청이 없는 한 범위에서 제외합니다.
