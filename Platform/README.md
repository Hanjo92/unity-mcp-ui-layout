...
- prefer stack-native layout relationships over raw pixels
- establish top-level region ownership before leaf-level tuning
- analyze mockups into a neutral layer-to-layout tree before stack-specific realization
- realize the tree with the selected stack's native hierarchy, styling, and reuse mechanisms
- add a screen host only when the selected runtime path requires one
...
- raw pixel보다 stack-native layout 관계를 우선합니다
- leaf-level 조정보다 먼저 top-level 영역 소유 관계를 정합니다
- 목업이나 UI 시안은 스택별 realization 전에 중립 layer-to-layout tree로 먼저 분석합니다
- 선택한 스택의 native hierarchy, styling, reuse 수단으로 트리를 realization합니다
- 선택한 runtime 경로에 screen host가 필요할 때만 추가합니다
...
## UGUI Adapter Rules / UGUI 어댑터 규칙
...
## UI Toolkit Adapter Rules / UI Toolkit 어댑터 규칙

- UI Toolkit realization maps the neutral tree to a visual tree, UXML, USS, and `VisualTreeAsset` template intent.
- Add a host GameObject/UIDocument only when a runtime host is needed; reusable template intent does not require a host by default.
- UI Toolkit realization은 중립 트리를 visual tree, UXML, USS, `VisualTreeAsset` template 의도로 옮깁니다.
- runtime host가 필요할 때만 host GameObject/UIDocument를 추가하며, 재사용 template 의도만으로 host를 만들지 않습니다.
