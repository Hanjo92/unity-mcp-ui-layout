...
- Did the intake identify active scene, active UI root, UI stack, stack-specific root settings, root layout owners, screenshot resolution, screenshot path, and console state?
- Were bounds, masks or clipping, scroll owners, reusable asset links, and key text overflow/wrap settings inspected, plus UGUI anchors/pivots/layout components or UI Toolkit flex/style owners as selected?
- Were referenced visual assets, styles, reusable templates, and shared asset markers inspected when asset-aware mode was active?
...
- Are the major top-level regions grouped by parent ownership before leaf-level adjustments?
...
- Are size changes controlled by the selected stack's root scaling and container rules: UGUI `CanvasScaler`/anchors or UI Toolkit panel/flex/style ownership?
...
- Were repeated structures turned into UGUI prefabs or UI Toolkit UXML/`VisualTreeAsset` templates with USS classes where appropriate?
...
Use this check when a mockup, screenshot, reference image, or UI 시안 drove item placement, icon/card/slot/button sizing, or reusable unit creation.

Ask:

- Did each split runtime or repeated item record a source rect from the mockup image?
- Was that source rect converted to a normalized rect before stack-specific local placement or sizing values were chosen?
- Is the implemented size controlled by the parent owner, named fit mode, and stack-specific realization rather than unrelated full-screen offsets?
- For UGUI, do anchors and layout components realize the item fit; for UI Toolkit, do flex/percentage/style owners or deliberate absolute overlay rules realize it?
...
- Does every mockup-derived crop reference an `asset_plan`, and did existing stack-appropriate visual assets or reusable templates stay preferred when available?
...
## 6. Stack-Specific Asset Type Check

Ask:

- For UGUI, are ordinary static visuals using `Image` and sprite-backed assets, with `RawImage` reserved for a clear runtime texture source such as `RenderTexture`, video, or generated texture data?
- For UI Toolkit, are ordinary static visuals using style backgrounds or resolved visual assets appropriate to the visual tree and USS ownership?
- Did any static icon, panel, slot, or button art bypass the selected stack's normal asset workflow without a good reason?

If the answer is no, convert the asset usage back toward the selected stack's normal visual-asset workflow before calling the UI done.
...
- For UGUI, were anchors and pivots chosen from UI role, constraints, growth, or motion intent; for UI Toolkit, were flex/percentage rules or deliberate absolute overlays chosen from the same intent?
