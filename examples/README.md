...
4. `prefab-from-mockup-example.md` if a mockup screenshot, dropped design image, or UI 시안 should become Unity UI prefabs
5. `mockup-resolution-example.md` if the mockup's native pixel resolution should drive planning
6. `mockup-decomposition-example.md` if the main question is what should stay baked, split, or become reusable
7. `design-md-layout-example.md` if a new screen should follow `DESIGN.md` tokens and prose intent
8. `design-md-repair-example.md` if an existing screen should be repaired while preserving `DESIGN.md`
9. `stitch-html-to-ugui-example.md` if a Stitch HTML/CSS export should be treated as the hierarchy source for a UGUI conversion
10. `figma-node-tree-to-ugui-example.md` if a Figma node/component-tree export should drive reusable UGUI hierarchy decisions
11. `current-vs-mockup-example.md` if the screen already exists and should be compared against a reference image first
12. `mobile-safe-area-mockup-example.md` if the mockup ignores notch or home-indicator constraints
13. `mobile-device-profile-verification-example.md` if a mobile-first screen needs named profile coverage before approval
14. `popup-safe-area-example.md` if mobile safe area and modal structure matter
15. `settings-dialog-example.md` if the screen is a dense options or preferences dialog
16. `responsive-split-pane-example.md` if the screen uses a left/right split or tablet-capable dashboard layout
17. `tabbed-detail-screen-example.md` if tabs, filters, or category buttons switch the visible content
18. `scroll-view-example.md` if the core challenge is scroll ownership plus reusable repeated rows or cards
19. `repair-one-region-example.md` if the request should stay bounded to one named region
20. `repair-asset-aware-reuse-example.md` if a repair may need prefab reuse, variants, wrappers, or shared-asset impact checks
21. `asset-naming-example.md` if the task also needs shared-versus-screen asset cleanup
22. `localized-screen-example.md` if the screen must survive both short English and longer localized strings
23. `long-labels-and-counters-example.md` if long labels, body text, and number growth compete for the same layout
24. `shared-asset-safety-example.md` if a repair might touch shared prefabs or other shared UI assets
25. `shared-asset-verification-example.md` if you need a concrete "check another usage first" prompt for shared assets
26. `ui-toolkit-example.md` if the target screen is clearly driven by `UIDocument`, `UXML`, and `USS`
